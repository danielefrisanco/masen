 ...
 *
 * **Two were still missing after Temaki, and they are drawn in `icons/`:**
 * `pipeline` — Temaki's `pipe` is a tobacco pipe — and `conflict`, for which
 * `ruins` had been a proxy rather than an answer. Both are on the same grid,
 * pass the same checks below, and are dedicated to the public domain, so the
 * vocabulary keeps one licence. Two drawings, not a set, which is what the
 * plan came down to once the rest was measured.
 */
...
const SETS = [
  ...
  {
    // Drawn here, CC0, because nothing public-domain carried them. Read through
    // the same checks as the vendored sets: a hand-drawn glyph off the grid is
    // the same defect as a Temaki one.
    label: "drawn here",
    dir: "icons",
    groups: {
      "energy and extraction": ["pipeline"],
      "borders and conflict": ["conflict"],
    },
  },
];
...
const counts = SETS.map((s) => {
  const n = [...seen.values()].filter((l) => l === s.label).length;
  return s.dir.startsWith("node_modules") ? `${n} from ${s.label}` : `${n} ${s.label}`;
}).join(", ");

await writeFile(
  OUT,
  `/**
 * Maki and Temaki, inlined, plus the two neither carried.
 *
 * Generated by \`scripts/build-icons.mjs\` — do not edit by hand. Both sets are
 * CC0, and so are the two glyphs drawn in \`icons/\`, which is the reason they
 * are what this library uses: a set requiring attribution would propagate that
 * obligation into every map anyone generates.
 ...
 * most of the geopolitical half a news map runs on, which the plan spent six
 * phases believing would have to be drawn by hand. Two of it did: \`pipeline\`
 * (Temaki's \`pipe\` is a tobacco pipe) and \`conflict\`, both on the same grid.
 */
...
console.log(`  ${OUT}  ${total} icons — ${counts} (all CC0)`);
